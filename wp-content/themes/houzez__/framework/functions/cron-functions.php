        ...
        if ( sizeof ( $results ) !== 0 ) :
            ...
            foreach ( $results as $houzez_saved_search ) :
                ...
                if ($user_email != '' && $data['mail_content'] != '') :
                    ...
                    $value_message = get_option('houzez_match_submission_message');
                    $value_subject = get_option('houzez_match_submission_subject');
        ...
        if($prop_selection->have_posts()){
            ...
            while ($prop_selection->have_posts()): $prop_selection->the_post();
                ...
                if( !empty( $prop_bed ) ) {
                    ...
                    $prop_bed_label = ($prop_bed > 1 ) ? get_option('houzez_glc_beds', 'Beds') : get_option('houzez_glc_bed', 'Bed');

                    $beds = '<span>'.$prop_bed.'</span> <span class="item-amenities-text">'.$prop_bed_label.'</span> ';
                }
                if( !empty( $prop_bath ) ) {
                    $prop_bath = esc_attr( $prop_bath );
                    $prop_bath_label = ($prop_bath > 1 ) ? get_option('houzez_glc_baths', 'Baths') : get_option('houzez_glc_bath', 'Bath');
